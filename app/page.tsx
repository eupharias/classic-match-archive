...
function AddMatch({data,onSave}:{data:TrackerData,onSave:(m:Match,p:Performance[])=>Promise<void>}){
  ...
  const [date,setDate]=useState(new Date().toISOString().slice(0,10)); const [side,setSide]=useState('Blue'); const [result,setResult]=useState('Win'); const [duration,setDuration]=useState('0:30:00'); const [notes,setNotes]=useState('');
