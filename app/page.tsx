...
function RolesArchive({data}:{data:TrackerData}) {
  ...
  const rankings=data.players.map(player=>{
    ...
    const bestChampion=Object.entries(championCounts).sort((a,b)=>b[1]-a[1]||a[0].localeCompare(b[0]))[0];
    const champion=bestChampion?.[0]??"—";
    ...
    const totals=championRows.reduce((acc,p)=>({kills:acc.kills+p.kills,deaths:acc.deaths+p.deaths,assists:acc.assists+p.assists,cs:acc.cs+p.cs,vision:acc.vision+p.vision,minutes:acc.minutes+(data.matches.find(m=>m.id===p.matchId)?.duration??0)}),{kills:0,deaths:0,assists:0,cs:0,vision:0,minutes:0});
    return {player,roleGames:roleRows.length,champion,championGames:championRows.length,winRate:wins/Math.max(1,championRows.length),kda:(totals.kills+totals.assists)/Math.max(1,totals.deaths),csPerMinute:totals.cs/Math.max(1,totals.minutes),visionPerMinute:totals.vision/Math.max(1,totals.minutes)};
  }).filter(row=>row.roleGames>0).sort((a,b)=>b.roleGames-a.roleGames||b.championGames-a.championGames||a.player.localeCompare(b.player));

  return <>
    <div className="role-archive-tabs" role="tablist" aria-label="Role archive">{roleOrder.map(role=><button type="button" role="tab" aria-selected={archiveRole===role} className={archiveRole===role?'active':''} onClick={()=>setArchiveRole(role)} key={role}><RoleLabel role={role}/></button>)}</div>
    <section className="panel role-rankings"><div className="panel-head"><div><p>ROLE ARCHIVE</p><h3><RoleLabel role={archiveRole}/> player activity</h3></div><span>Sorted by total matches in role</span></div>
      <div className="role-ranking-groups"><span>ROLE ACTIVITY</span><span>MOST-PLAYED CHAMPION PERFORMANCE</span></div>
      <div className="role-ranking-head"><span>#</span><span>PLAYER</span><span>ROLE MATCHES</span><span>MOST-PLAYED CHAMPION</span><span>CHAMPION MATCHES</span><span>CHAMPION WR</span><span>CHAMPION KDA</span><span>CHAMPION CS / MIN</span><span>CHAMPION VISION / MIN</span></div>
      <div className="role-ranking-list">{rankings.map((row,index)=><article key={row.player}><span className="role-rank">{String(index+1).padStart(2,'0')}</span><b>{row.player}</b><div><small>ROLE MATCHES</small><strong>{row.roleGames}</strong></div><div className="role-best-champion"><img src={championIcon(row.champion)} alt=""/><span><strong>{row.champion}</strong><small>Most played by match count</small></span></div><div><small>CHAMPION MATCHES</small><strong>{row.championGames}</strong></div><div><small>CHAMPION WR</small><strong>{pct(row.winRate)}</strong></div><div><small>CHAMPION KDA</small><strong>{num(row.kda,2)}</strong></div><div><small>CHAMPION CS / MIN</small><strong>{num(row.csPerMinute,2)}</strong></div><div><small>CHAMPION VISION / MIN</small><strong>{num(row.visionPerMinute,2)}</strong></div></article>)}</div>
...
export default function Home() {
  ...
  return <div className="app-shell">
    ...
    <main className="main">
      ...
      {tab==="players" && <>
        <div className="player-tabs" role="tablist">{data.players.map(p=><button role="tab" aria-selected={p===player} className={p===player?'active':''} onClick={()=>{setPlayer(p);setExpandedChampion(null);setSelectedRole(null);setShowAllChampions(false)}} key={p}>{p}</button>)}</div>
