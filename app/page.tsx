...
export default function Home() {
  ...
  return <div className="app-shell">
    ...
    <main className="main">
      ...
      {tab==="players" && <>
        <div className="player-tabs" role="tablist">{data.players.map(p=><button role="tab" aria-selected={p===player} className={p===player?'active':''} onClick={()=>{setPlayer(p);setExpandedChampion(null);setSelectedRole(null);setShowAllChampions(false);setShowAllPlayerMatches(false);setPlayerMatchPage(0)}} key={p}>{p}</button>)}</div>
