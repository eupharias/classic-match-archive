...
function RolesArchive({data}:{data:TrackerData}) {
  ...
  return <>
    ...
    <section className="panel role-rankings"><div className="panel-head"><div><p>ROLE ARCHIVE</p><h3><RoleLabel role={archiveRole}/> player activity</h3></div><span>Sorted by total matches in role</span></div>
      <div className="role-ranking-head"><span>#</span><span>PLAYER</span><span>BEST CHAMPION</span><span>ROLE MATCHES</span><span>CHAMPION MATCHES</span><span>WIN RATE</span><span>KDA</span><span>CS / MIN</span><span>VISION / MIN</span></div>
      <div className="role-ranking-list">{rankings.map((row,index)=><article key={row.player}><span className="role-rank">{String(index+1).padStart(2,'0')}</span><b>{row.player}</b><div className="role-best-champion"><img src={championIcon(row.champion)} alt=""/><span><strong>{row.champion}</strong><small>Most played</small></span></div><div><small>ROLE MATCHES</small><strong>{row.roleGames}</strong></div><div><small>CHAMPION MATCHES</small><strong>{row.championGames}</strong></div><div><small>WIN RATE</small><strong>{pct(row.winRate)}</strong></div><div><small>KDA</small><strong>{num(row.kda,2)}</strong></div><div><small>CS / MIN</small><strong>{num(row.csPerMinute,2)}</strong></div><div><small>VISION / MIN</small><strong>{num(row.visionPerMinute,2)}</strong></div></article>)}</div>
