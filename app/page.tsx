  ...
  return <div className="app-shell">
    ...
    <main className="main">
      ...
      {tab==="players" && <>
        ...
        <section className="player-overall"><div className="panel role-summary"><div className="panel-head"><div><p>ROLE PERFORMANCE</p><h3>Overall role breakdown</h3></div></div><div className="role-summary-grid">{playerRoleStats.map(role=><article key={role.role}><div><b>{role.role}</b><span>{role.games} {role.games===1?'match':'matches'}</span></div><div><small>WIN RATE</small><strong>{role.games?pct(role.wins/role.games):'—'}</strong></div><div><small>MOST PLAYED</small><strong>{role.favorite}</strong>{role.favoriteGames>0&&<span>{role.favoriteGames} {role.favoriteGames===1?'game':'games'}</span>}</div></article>)}</div></div><div className="panel career-totals"><div className="panel-head"><div><p>CAREER TOTALS</p><h3>Combat record</h3></div></div><div className="career-grid"><div><span>KILLS</span><b>{playerTotals.kills}</b></div><div><span>DEATHS</span><b>{playerTotals.deaths}</b></div><div><span>ASSISTS</span><b>{playerTotals.assists}</b></div><div><span>TOTAL KDA</span><b>{num((playerTotals.kills+playerTotals.assists)/Math.max(1,playerTotals.deaths),2)}</b></div></div></div></section>
