...
type ComparisonColumn=ChampionStatKey|"recordedMatches";
const comparisonColumnLabel=(column:ComparisonColumn)=>column==="recordedMatches"?"Recorded Matches":statNames[column];

function ChampionComparisons({archive}:{archive:ArchiveData}){
  const classes=Array.from(new Set(champions.flatMap(champion=>champion.tags))).sort();
  const [selectedClasses,setSelectedClasses]=useState<string[]>([]);
  const [visibleColumns,setVisibleColumns]=useState<ComparisonColumn[]>(["recordedMatches",...championStatKeys]);
  const [settingsOpen,setSettingsOpen]=useState(false);
  const [expanded,setExpanded]=useState(false);const [page,setPage]=useState(0);
  const [sort,setSort]=useState<{column:"name"|ComparisonColumn;direction:1|-1}>({column:"name",direction:1});
  const recordedCounts=useMemo(()=>archive.performances.filter(row=>row.tracked).reduce((counts,row)=>{counts.set(row.champion,(counts.get(row.champion)??0)+1);return counts},new Map<string,number>()),[archive.performances]);
  const filtered=champions.map(champion=>({champion,recordedMatches:recordedCounts.get(archiveChampionName(champion.name))??0})).filter(row=>!selectedClasses.length||row.champion.tags.some(tag=>selectedClasses.includes(tag))).sort((a,b)=>{const av=sort.column==="name"?a.champion.name.toLowerCase():sort.column==="recordedMatches"?a.recordedMatches:Number(a.champion.stats[sort.column]??0);const bv=sort.column==="name"?b.champion.name.toLowerCase():sort.column==="recordedMatches"?b.recordedMatches:Number(b.champion.stats[sort.column]??0);return (typeof av==="string"?av.localeCompare(String(bv)):av-Number(bv))*sort.direction});
  const pageCount=Math.max(1,Math.ceil(filtered.length/20));const visibleRows=expanded?filtered.slice(page*20,page*20+20):filtered.slice(0,10);
  const toggleClass=(value:string)=>{setSelectedClasses(current=>current.includes(value)?current.filter(item=>item!==value):[...current,value]);setExpanded(false);setPage(0)};
  const clearClasses=()=>{setSelectedClasses([]);setExpanded(false);setPage(0)};
  const toggleColumn=(value:ComparisonColumn)=>setVisibleColumns(current=>current.includes(value)?current.filter(item=>item!==value):[...current,value]);
  const setColumn=(column:"name"|ComparisonColumn)=>{setSort(current=>({column,direction:current.column===column?(current.direction===1?-1:1):column==="name"?1:-1}));setPage(0)};
  const arrow=(column:"name"|ComparisonColumn)=>sort.column===column?(sort.direction===1?"▲":"▼"):"↕";
  return <section className="champion-comparisons">
    <div className="comparison-controls"><div><span>FILTER BY CLASS · OR LOGIC</span><div className="comparison-class-filters">{classes.map(value=><button type="button" className={selectedClasses.includes(value)?"active":""} aria-pressed={selectedClasses.includes(value)} onClick={()=>toggleClass(value)} key={value}>{value}</button>)}{selectedClasses.length>0&&<button type="button" className="clear" onClick={clearClasses}>Clear</button>}</div></div><b>{filtered.length} champions</b></div>
    <div className="comparison-table-wrap">
      <div className="comparison-settings"><button type="button" aria-label="Configure comparison columns" aria-expanded={settingsOpen} onClick={()=>setSettingsOpen(value=>!value)}>⚙</button>{settingsOpen&&<aside><header><b>TABLE COLUMNS</b><button type="button" onClick={()=>setSettingsOpen(false)}>×</button></header><p>Select the statistics to display.</p><div>{(["recordedMatches",...championStatKeys] as ComparisonColumn[]).map(key=><label key={key}><input type="checkbox" checked={visibleColumns.includes(key)} onChange={()=>toggleColumn(key)}/><span>{comparisonColumnLabel(key)}</span></label>)}</div><footer><button type="button" onClick={()=>setVisibleColumns(["recordedMatches",...championStatKeys])}>Show all</button><button type="button" onClick={()=>setVisibleColumns([])}>Hide all stats</button></footer></aside>}</div>
      <div className="comparison-table" style={{"--comparison-columns":`230px 150px ${visibleColumns.map(()=>"minmax(118px,1fr)").join(" ")}`} as CSSProperties}>
        <div className="comparison-heading"><button type="button" onClick={()=>setColumn("name")}>CHAMPION <i>{arrow("name")}</i></button><span>CLASS</span>{visibleColumns.map(key=><button type="button" onClick={()=>setColumn(key)} key={key}>{comparisonColumnLabel(key).toUpperCase()} <i>{arrow(key)}</i></button>)}</div>
        {visibleRows.map(({champion,recordedMatches})=><div className="comparison-row" key={champion.id}><div className="comparison-champion"><img src={championPortrait(champion)} alt=""/><b>{champion.name}</b></div><span>{champion.tags.join(" · ")}</span>{visibleColumns.map(key=><strong key={key}>{key==="recordedMatches"?recordedMatches:Number(champion.stats[key]??0).toLocaleString(undefined,{maximumFractionDigits:3})}</strong>)}</div>)}
      </div>
    </div>
    {filtered.length>10&&<div className="comparison-pagination">{!expanded?<button type="button" onClick={()=>{setExpanded(true);setPage(0)}}>Show top 20 and pagination</button>:<><button type="button" disabled={page===0} onClick={()=>setPage(value=>Math.max(0,value-1))}>← Previous</button><span>Page {page+1} of {pageCount} · {visibleRows.length} champions shown</span><button type="button" disabled={page>=pageCount-1} onClick={()=>setPage(value=>Math.min(pageCount-1,value+1))}>Next →</button><button type="button" onClick={()=>{setExpanded(false);setPage(0)}}>Show top 10</button></>}</div>}
  </section>
}
...
function ChampionsCollection({archive}:{archive:ArchiveData}) {
  ...
  const [itemSort,setItemSort]=useState<"frequency"|"winRate">("frequency");
  ...
  const trinketIds=new Set(items.filter(item=>item.tags?.includes("Trinket")).map(item=>Number(item.id)));
  const itemMatches=new Map<number,Set<number>>();const trinketMatches=new Map<number,Set<number>>();championRows.forEach(row=>row.items?.forEach(entry=>{const target=trinketIds.has(entry.itemId)?trinketMatches:itemMatches;const matches=target.get(entry.itemId)??new Set<number>();matches.add(row.matchId);target.set(entry.itemId,matches)}));
  const topItems=[...itemMatches.entries()].map(([id,matchIds])=>{const frequency=matchIds.size;const wins=[...matchIds].filter(matchId=>groupMatches.find(match=>match.id===matchId)?.result==="Win").length;return {id,frequency,winRate:wins/Math.max(1,frequency)}}).sort((a,b)=>itemSort==="winRate"?b.winRate-a.winRate||b.frequency-a.frequency:b.frequency-a.frequency||b.winRate-a.winRate).slice(0,6);
  const trinketFrequency=[773340,773348].map(id=>[id,trinketMatches.get(id)?.size??0] as [number,number]).sort((a,b)=>b[1]-a[1]||a[0]-b[0]);
  ...
  return <section className="panel collection-panel champion-collection">
    ...
    {championView==="comparisons"?<ChampionComparisons archive={archive}/>:<>
    ...
    <section className="champion-archive-stats"><div className="champion-section-head"><p>ARCHIVE STATISTICS</p><h4>{selected.name} in WREQ group matches</h4></div><div className="champion-archive-kpis"><div><span>MATCHES</span><b>{championMatches.length}</b></div><div><span>WIN RATE</span><b>{championMatches.length?`${Math.round(championWins/championMatches.length*100)}%`:"—"}</b></div><div><span>PLAYERS</span><b>{new Set(championRows.map(row=>row.player)).size}</b></div><div><span>ROLE DISTRIBUTION</span><b>{roleCounts.map(entry=>`${entry.name} ${entry.count}`).join(" · ")||"—"}</b></div></div><div className="champion-inventory-breakdown"><div className="champion-top-items"><header><span>FINAL ITEMS</span><label>SORT BY<select value={itemSort} onChange={event=>setItemSort(event.target.value as "frequency"|"winRate")}><option value="frequency">Frequency</option><option value="winRate">Winrate</option></select></label></header><div>{topItems.map(({id,frequency,winRate})=>{const item=(catalog.data as Record<string,ItemRecord>)[String(id)];return <span key={id}><img src={imageUrl(String(id))} alt=""/><b>{item?.name??`Item ${id}`}</b><small>{frequency} {frequency===1?"match":"matches"}</small><em>{Math.round(winRate*100)}% win rate</em></span>})}{!topItems.length&&<small className="champion-items-empty">No final items recorded.</small>}</div></div><div className="champion-trinkets"><span>TRINKET FREQUENCY</span><div>{trinketFrequency.map(([id,count])=>{const item=(catalog.data as Record<string,ItemRecord>)[String(id)];return <span key={id}><img src={imageUrl(String(id))} alt=""/><b>{item?.name??`Item ${id}`}</b><small>{count} {count===1?"match":"matches"}</small></span>})}</div></div></div>
