...
function ChampionComparisons(){
  ...
  return <section className="champion-comparisons">
    ...
    <div className="comparison-table-wrap">
      ...
      <div className="comparison-table" style={{"--comparison-columns":`230px 150px ${visibleStats.map(()=>"minmax(118px,1fr)").join(" ")}`} as CSSProperties}>
        <div className="comparison-heading"><button type="button" onClick={()=>setColumn("name")}>CHAMPION <i>{arrow("name")}</i></button><span>CLASS</span>{visibleStats.map(key=><button type="button" onClick={()=>setColumn(key)} key={key}>{statNames[key].toUpperCase()} <i>{arrow(key)}</i></button>)}</div>
