...
function ChampionsCollection({archive}:{archive:ArchiveData}) {
  ...
  const itemCounts=new Map<number,number>();const trinketMatches=new Map<number,Set<number>>();championRows.forEach(row=>row.items?.forEach(entry=>{if(trinketIds.has(entry.itemId)){const matches=trinketMatches.get(entry.itemId)??new Set<number>();matches.add(row.matchId);trinketMatches.set(entry.itemId,matches)}else itemCounts.set(entry.itemId,(itemCounts.get(entry.itemId)??0)+entry.quantity)}));
  ...
  return <section className="panel collection-panel champion-collection">
    ...
    <section className="champion-archive-stats"><div className="champion-section-head"><p>ARCHIVE STATISTICS</p><h4>{selected.name} in WREQ group matches</h4></div><div className="champion-archive-kpis"><div><span>MATCHES</span><b>{championMatches.length}</b></div><div><span>WIN RATE</span><b>{championMatches.length?`${Math.round(championWins/championMatches.length*100)}%`:"—"}</b></div><div><span>PLAYERS</span><b>{new Set(championRows.map(row=>row.player)).size}</b></div><div><span>ROLE DISTRIBUTION</span><b>{roleCounts.map(entry=>`${entry.name} ${entry.count}`).join(" · ")||"—"}</b></div></div><div className="champion-inventory-breakdown"><div className="champion-top-items"><span>MOST RECORDED FINAL ITEMS</span><div>{topItems.map(([id,count])=>{const item=(catalog.data as Record<string,ItemRecord>)[String(id)];return <span key={id}><img src={imageUrl(String(id))} alt=""/><b>{item?.name??`Item ${id}`}</b><small>{count} recorded</small></span>})}{!topItems.length&&<small className="champion-items-empty">No final items recorded.</small>}</div></div><div className="champion-trinkets"><span>TRINKET FREQUENCY</span><div>{trinketFrequency.map(([id,count])=>{const item=(catalog.data as Record<string,ItemRecord>)[String(id)];return <span key={id}><img src={imageUrl(String(id))} alt=""/><b>{item?.name??`Item ${id}`}</b><small>{count} {count===1?"match":"matches"}</small></span>})}</div></div></div>
